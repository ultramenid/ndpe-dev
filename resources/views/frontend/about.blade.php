                ...
                <div class="flex sm:flex-row flex-col justify-between sm:space-x-12 space-x-0 sm:space-y-0 space-y-6 mt-12">
                    ...
                    <div class="sm:w-7/12 w-full">
                        <div class="flex flex-col space-y-6" x-show="tabs === 'theplatform' ">
                            ...
                            <p class="leading-relaxed">
                                {{__('We hope that this website will become a go-to tool for global buyers and investors to get')}}<a href="{{ route('update', app()->getlocale() )}}" class="text-red-500"> {{__('update')}}</a> {{__('information on the latest issues and understand the performance of')}}<a href="{{ route('performance', app()->getlocale() )}}" class="text-red-500"> {{__('seven key corporate groups')}} </a>{{__('producing and trading commodities against the')}} <a href="{{ route('performance', app()->getlocale() )}}"  class="text-red-500">{{__('benchmark')}}</a>
                                {{__('set up by NGOs so they can take informed decisions to implement their responsible commodity sourcing policies.')}}
                            </p>
                            <p class="leading-relaxed">
